...
class UMS_Controller extends CI_Controller
{
	...
	*/
	public function check_que_appointment_doctor_wts()
	{
		...
		$minute_check = $this->config->item('wts_time_min_alert');
		...
		foreach ($noti_depts as $noti_dept) {
			...
			$this->m_wts_notifications_department->ntdp_apm_id = $noti_dept['ntr_apm_id'];
			...
			$end_date_check->modify("-$minute_check minutes"); // for wts_notifications_status = 3 ใกล้หมดเวลา
		...
		if (!empty($result)) {
			switch ($ntdp_sta_id) {
				...
				case 3:
					...
					$pt_full_name = $result->pt_prefix . " " . $result->pt_fname . " " . $result->pt_lname;
					$detail = "อีก $minute_check นาทีจะหมดเวลาการรักษาที่กำหนดไว้ <br>ผู้ป่วย: " . $pt_full_name;
					return ['subject' => $subject, 'detail' => $detail];
					break;
			}
		}
