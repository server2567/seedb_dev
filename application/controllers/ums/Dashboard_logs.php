...
/*
* User
* Main controller of Dashboard_logs
* @input -
* $output -
* @author Areerat Pongurai
* @Create Date 119/06/2024
*/

if ( ! defined('BASEPATH')) exit('No direct script access allowed');
require_once('UMS_Controller.php');

class Dashboard_logs extends UMS_Controller{
	...
	/*
	* index
	* Index controller of Dashboard_logs
	* @input -
	* $output log list
	* @author Areerat Pongurai
	...
	*/
	public function index() {
		...
		$logs = $this->m_ums_logs->get_all($filter)->result_array();

		// encrypt id
		$names = ['log_id']; // object name need to encrypt 
		$data['logs'] = encrypt_arr_obj_id($logs, $names);

		$data['session_mn_active_url'] = $this->uri->segment(1) . '/' . $this->uri->segment(2); // set session_mn_active_url / breadcrumb
		$data['status_response'] = $this->config->item('status_response_show');
		$this->output('ums/dashboard/v_dashboard_logs_show',$data);
	}
}
?>
