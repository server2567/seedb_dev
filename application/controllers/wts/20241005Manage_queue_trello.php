...
class Manage_queue_trello_20241005 extends WTS_Controller
{
	...
	public function Manage_queue_trello_get_ques()
	{
		...
		foreach ($result as $key => $task) {
			...
			if($task['apm_ps_id'] == null || $task['apm_ps_id'] == ''){
				$btn = '<button class="btn btn-success" onclick="select_doctor()" style="margin-right:5px"><i class="bi bi-person"> </i></button>';
	...
	*/
	...
	public function get_new_patient_que()
	{
		...
		$number_que = $this->m_que_appointment->get_patient_no_que($date_format)->result();
		$response = ['num_row' => count($number_que)];
		echo json_encode($response);
	}
	public function get_doctor_by_stde_select(){
