...
class PHPExcel_Calculation
{
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        $formula = trim($formula);
        if ($formula[0] != '=') { // Changed from $formula{0} to $formula[0]
            return self::wrapResult($formula);
        }
        $formula = ltrim(substr($formula, 1));
        if (!isset($formula[0])) { // Changed from $formula{0} to $formula[0]
            return self::wrapResult($formula);
        }
        ...
        if (($wsTitle[0] !== "\x00") && ($this->cyclicReferenceStack->onStack($wsCellReference))) { // Changed from $wsTitle{0} to $wsTitle[0]
    ...
    private function showTypeDetails($value)
    {
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            if ($value === null) {
                ...
                } elseif ($value[0] == '#') { // Changed from $value{0} to $value[0]
    ...
    private function validateBinaryOperand($cellID, &$operand, &$stack)
    {
        ...
        if (is_string($operand)) {
            // Start by stripping off the quotation marks
            if ($operand > '' && $operand[0] == '"') { // แก้ไขที่นี่
                $operand = self::unwrapResult($operand);
            }
            // If the string is a numeric value, we treat it as a numeric
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        if (is_string($operand1) && $operand1 > '' && $operand1[0] == '"') { // แก้ไขที่นี่
            $operand1 = self::unwrapResult($operand1);
        }
        if (is_string($operand2) && $operand2 > '' && $operand2[0] == '"') { // แก้ไขที่นี่
            $operand2 = self::unwrapResult($operand2);
        }

        // ใช้การเปรียบเทียบไม่สนใจตัวพิมพ์เล็กหากไม่อยู่ในโหมด OpenOffice
