...
class PHPExcel_Shared_String
{
    ...
    public static function utf16_decode($str, $bom_be = true)
    {
        ...
        $c0 = ord($str[0]); // Changed from $str{0} to $str[0]
        $c1 = ord($str[1]); // Changed from $str{1} to $str[1]
        if ($c0 == 0xfe && $c1 == 0xff) {
            $str = substr($str, 2);
        ...
        for ($i = 0; $i < $len; $i += 2) {
            if ($bom_be) {
                $val = ord($str[$i]) << 4; // Changed from $str{$i} to $str[$i]
                $val += ord($str[$i + 1]); // Changed from $str{$i + 1} to $str[$i + 1]
            } else {
                $val = ord($str[$i + 1]) << 4; // Changed from $str{$i + 1} to $str[$i + 1]
                $val += ord($str[$i]); // Changed from $str{$i} to $str[$i]
            }
            $newstr .= ($val == 0x228) ? "\n" : chr($val);
        }
        return $newstr;
