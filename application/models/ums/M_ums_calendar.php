	...
	function get_calendar_person_date($start_date, $end_date, $ps_id){
		$sql = "
			...
			SELECT 
				...
				twpp.twpp_end_time AS clnd_end_time, 
				NULL AS clnd_parent_id,
				IF(hire.hire_is_medical = 'M', 3, 2) AS id, 
				clt.clt_name AS category, 
				clt.clt_color AS color,
				CONCAT(pf_name, ps_fname, ' ', ps_lname) AS create_calender_by
			FROM ".$this->hr_db.".hr_timework_person_plan AS twpp
			LEFT JOIN ".$this->hr_db.".hr_person ON twpp.twpp_ps_id = ps_id
			LEFT JOIN ".$this->hr_db.".hr_base_prefix ON ps_pf_id = pf_id
			LEFT JOIN " . $this->hr_db . ".hr_timework_attendance_config as twac
                ON twac.twac_id = twpp.twpp_twac_id
			LEFT JOIN " . $this->eqs_db . ".eqs_room as rm
				ON rm.rm_id = twpp.twpp_rm_id
			LEFT JOIN " . $this->eqs_db . ".eqs_building as bd
				ON rm.rm_bd_id = bd.bd_id
			LEFT JOIN " . $this->hr_db . ".hr_person_position AS pos 
				ON pos.pos_ps_id = ps_id
			...
			SELECT 
				lhis.lhis_id AS clnd_id,
				lhis.lhis_ps_id AS clnd_ps_id,
				CONCAT(
					le.leave_name
				...
				lhis.lhis_start_date AS clnd_start_date, 
				lhis.lhis_end_date AS clnd_end_date, 
				'00:00:00' AS clnd_start_time, 
				'00:00:00' AS clnd_end_time, 
				NULL AS clnd_parent_id,
				4 AS id, 
				...
				clt.clt_color AS color,
				CONCAT(pf_name, ps_fname, ' ', ps_lname) AS create_calender_by
			FROM " . $this->hr_db . ".hr_leave_history as lhis
			LEFT JOIN ".$this->hr_db.".hr_leave_history_detail as lhde ON lhde.lhde_lhis_id = lhis.lhis_id
			LEFT JOIN ".$this->hr_db.".hr_person ON lhis.lhis_ps_id = ps_id
			LEFT JOIN ".$this->hr_db.".hr_base_prefix ON ps_pf_id = pf_id
			LEFT JOIN ".$this->hr_db.".hr_leave as le ON lhis.lhis_leave_id = le.leave_id
			LEFT JOIN ".$this->ums_db.".ums_calendar_type AS clt ON clt.clt_id = 4
			
			WHERE lhis.lhis_start_date >= '{$start_date}' AND lhis.lhis_end_date <= '{$end_date}' 
				AND lhis.lhis_ps_id = {$ps_id} AND lhis.lhis_status = 'Y'
	...
	*/
	function get_group_calendar_type_person_date($start_date, $end_date){
		$sql = "SELECT 
				...
				LEFT JOIN ".$this->ums_db.".ums_calendar_type AS clt ON clt.clt_id = 4
				WHERE lhis.lhis_start_date >= '{$start_date}' AND lhis.lhis_end_date <= '{$end_date}' 
					AND lhis.lhis_ps_id = ".$this->session->userdata('us_ps_id')." AND lhis.lhis_status = 'Y'
				GROUP BY id
