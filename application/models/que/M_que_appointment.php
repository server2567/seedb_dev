...
class M_que_appointment extends Da_que_appointment
{
	...
	*/
    ...
    function get_doctors_by_department($stde_id = null, $show_all = false)
    {
        ...
        $sql = "SELECT * , CONCAT(hr_base_prefix.pf_name_abbr, '', hr_person.ps_fname) AS ps_name
        FROM $this->hr_db.hr_structure_person 
    ...
    function get_doctors_by_department_que($stde_id = null, $show_all = false)
    {
        $us_dp_id = $this->session->userdata('us_dp_id');
        if (!$show_all && $stde_id !== null) {
            $stde_id_sql = "AND stde_id = '" . $stde_id . "' ";
    ...
    public function get_patient_no_que_old($date_format)
    {
        ...
        LEFT JOIN (
            SELECT t1.ntdp_apm_id, t1.ntdp_loc_id, t1.ntdp_date_start, t1.ntdp_time_start
            FROM see_wtsdb.wts_notifications_department t1
            INNER JOIN (
                SELECT ntdp_apm_id, 
            ...
            ON t1.ntdp_apm_id = t2.ntdp_apm_id
            AND t1.ntdp_date_start = t2.max_ntdp_date_start
            AND t1.ntdp_time_start = t2.max_ntdp_time_start
        ) ntdp ON ntdp.ntdp_apm_id = que.apm_id
        WHERE que.apm_sta_id = '4' 
        AND ntdp.ntdp_loc_id = '6' 
        AND qus.qus_id IS NULL 
        AND que.apm_date = '$date_format' 
        GROUP BY que.apm_id";
        $query = $this->que->query($sql);
        return $query->result(); // Return result as array
    }
    public function get_patient_no_que($date_format, $wait_que)
    {
        if ($wait_que) {
            $wait_que_string = implode(",", $wait_que);
        }
        $sql = "SELECT * FROM $this->que_db.que_appointment as que
        LEFT JOIN $this->wts_db.wts_queue_seq as qus on qus.qus_apm_id = que.apm_id
        LEFT JOIN (
            ...
            ON t1.ntdp_date_start = t2.max_ntdp_date_start
            AND t1.ntdp_time_start = t2.max_ntdp_time_start
        ) ntdp ON ntdp.ntdp_apm_id = que.apm_id
        WHERE que.apm_sta_id = '4' 
        AND ntdp.ntdp_loc_id = '6' 
        AND qus.qus_id IS NULL 
        AND que.apm_date = '$date_format' " .
            (!isset($wait_que) ? '' : "AND que.apm_visit NOT IN ($wait_que_string)") . "GROUP BY que.apm_id";
        $query = $this->que->query($sql);
        return $query->result(); // Return result as array
    }
    public function reset_report($date_format)
    {
        // $sql = "DELETE FROM see_wtsdb.wts_queue_seq 
        //     WHERE qus_announce_id IS NOT NULL 
        //     AND qus_announce_id IN (
        //         SELECT apm_id
        //         FROM see_quedb.que_appointment
        //         WHERE apm_sta_id = 10 AND apm_date = '$date_format'
        //     )";
        // // รันคำสั่ง SQL
        // $query = $this->que->query($sql);
        $sql_select = "SELECT apm_id, qus_apm_id 
        FROM see_quedb.que_appointment 
        INNER JOIN see_wtsdb.wts_queue_seq 
        ON see_quedb.que_appointment.apm_id = see_wtsdb.wts_queue_seq.qus_announce_id
        WHERE see_quedb.que_appointment.apm_sta_id = 10 
        AND see_quedb.que_appointment.apm_date = '$date_format'";

        $query_select = $this->que->query($sql_select);
        $results = $query_select->result_array();

        if (!empty($results)) {
            // แปลง apm_id และ qus_apm_id เป็น array
            $apm_ids = implode(',', array_column($results, 'apm_id'));
            $qus_apm_ids = implode(',', array_column($results, 'qus_apm_id'));

            // ลบข้อมูลใน wts_queue_seq
            $sql_delete_wts = "DELETE FROM see_wtsdb.wts_queue_seq 
                WHERE qus_announce_id IN ($apm_ids)";
            $this->que->query($sql_delete_wts);

            // ลบข้อมูลใน que_appointment
            $sql_delete_apm = "DELETE FROM see_quedb.que_appointment 
                WHERE apm_id IN ($qus_apm_ids)";
            $this->que->query($sql_delete_apm);
        }
    }
    ...
    function insert_patient($identification, $pt_password, $pt_password_confirm, $pt_prefix, $pt_fname, $pt_lname, $pt_tel, $pt_email)
    {
        ...
        $this->ums->query($sql, array(
            ...
            $pt_password,
            $pt_password_confirm,
            $pt_prefix, // ข้อมูลส่วนตัว
            $pt_fname,
            $pt_lname,
            $pt_tel,
            $pt_email, // ข้อมูลส่วนตัว
            $this->session->userdata('us_id'),
            $pt_peregrine,
            $pt_passport // ผู้ใช้ที่สร้าง
        ));
    ...
    function update_announce()
    {
        ...
        $this->que->query($sql, array(
            ...
            $this->apm_ps_id,
            $this->apm_time,
            $this->apm_id  // Make sure to pass `apm_id`
        ));
    }
    ...
    */
    public function get_appointment_trello_wts_old($params)
    {
        // $ps_id = $this->session->userdata('us_ps_id');
        // $get_stde = $this->hr->query("SELECT * FROM hr_structure_detail 
        //                               LEFT JOIN hr_structure_person ON stdp_stde_id = stde_id 
        ...
        if (!empty($params['sta_id'])) {
             ...
             */
            if (!empty($where)) $where .= " AND ";
            $where .= " apm.apm_sta_id NOT IN ('1', '5', '9', '10','15') ";
        }
        ...
        $sql .= $order . " LIMIT 15";  // เพิ่ม LIMIT 30
