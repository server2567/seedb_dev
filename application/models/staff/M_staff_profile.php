	...
	*/
    function get_medical_profile($is_medical, $ps_id = null, $fl_name = null, $fl_stde = null)
    {
        if ($ps_id == null) {
            if (is_array($is_medical)) {
                ...
                if ($is_medical == 'lvl') {
                    $cond = "stuc.stuc_status = 1 AND ( stde.stde_level <= 2 OR psap.psap_admin_id = 30)";
                } else {
    ...
    function filter_profile_option($stuc_lvl)
    {
        ...
        if ($stuc_lvl == 'SM' || $stuc_lvl == 'M' || $stuc_lvl == 'N') {
            if ($stuc_lvl == 'M') {
                $where .= ($stuc_lvl == 'N' ? "( stde.stde_is_medical = 'Y' OR stde.stde_is_medical = 'N') AND " : "stde.stde_is_medical = 'Y' AND ");
            } else {
