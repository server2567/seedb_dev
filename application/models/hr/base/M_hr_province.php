...
*/
include_once("Da_hr_province.php");

class M_hr_province extends Da_hr_province {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_province` 
				SET `pv_active` = '".$active."' 
				WHERE `hr_base_province`.`pv_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		$sql = "SELECT * 
				...
				WHERE pv_active != '$delete'
				ORDER BY pv_active DESC,pv_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
