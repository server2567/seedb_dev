	...
	*/
	function get_all_by_active($delete = "2")
	{
		$sql = "SELECT * 
				...
				WHERE amph_active != '$delete'
				ORDER BY amph_active DESC,amph_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
	function get_amphur_by_provice($pv_id,$delete =2)
	{
		$sql = "SELECT * 
		FROM " . $this->hr_db . ".hr_base_amphur
		WHERE amph_pv_id = '$pv_id' AND amph_active != '$delete'";
		$query = $this->hr->query($sql);
		return $query;
	}
	/*
