...
*/
include_once("Da_hr_reward_type.php");

class m_hr_reward_type extends Da_hr_reward_type {
	...
	*/
	function update_active($status){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_reward_type` 
				SET `rwt_status` = '".$status."' 
				WHERE `hr_base_reward_type`.`rwt_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE rwt_active != '$delete'
		ORDER BY rwt_active DESC,rwt_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
