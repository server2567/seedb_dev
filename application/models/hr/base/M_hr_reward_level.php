...
*/
include_once("Da_hr_reward_level.php");

class m_hr_reward_level extends Da_hr_reward_level {
	...
	*/
	function update_active($status){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_reward_level` 
				SET `rwlv_status` = '".$status."' 
				WHERE `hr_base_reward_level`.`rwlv_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE rwlv_active != '$delete'
		ORDER BY rwlv_active DESC,rwlv_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
	function get_calendar_type(){
		$sql = "SELECT * 
				FROM ".$this->hr_db.".base_calendar_type";
		$query = $this->hr->query($sql);
		return $query;
	}
