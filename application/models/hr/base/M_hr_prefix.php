...
*/
include_once("Da_hr_prefix.php");

class M_hr_prefix extends Da_hr_prefix {
	...
	// public function setDynamicField($field, $value) {
    //     // ตรวจสอบว่าฟิลด์ที่ส่งเข้ามามีอยู่ในคลาสหรือไม่
	...
	function get_max_pf_seq_no() {
		...
		$sql = "SELECT MAX(`pf_seq_no`) AS max FROM ".$this->hr_db.".hr_base_prefix";
		$query = $this->hr->query($sql);
		if($query->row()){
	...
	*/
	function get_all_by_active($delete="2"){
		$sql = "SELECT * 
				FROM ".$this->hr_db.".hr_base_prefix 
				WHERE pf_active != '$delete'
				ORDER BY pf_active DESC,pf_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}	

	function get_gender($active="2"){
		$sql = "SELECT * 
				FROM ".$this->hr_db.".hr_base_gender 
				WHERE gd_active != '$active'";
		$query = $this->hr->query($sql);
		return $query;
	}
	...
	*/
	function get_prefix_pos() { 
		$sql = "SELECT * FROM ".$this->hr_db.".hr_base_prefix
				WHERE pf_name LIKE '%ศาสตราจารย์%' ";
		$query = $this->db->query($sql);		
		return $query ;
	}

	function get_prefix_all() { 
		$sql = "SELECT * FROM ".$this->hr_db.".hr_base_prefix
				WHERE pf_active = '1'";
		$query = $this->db->query($sql);		
		return $query ;
