...
*/
include_once("Da_hr_occupation.php");

class M_hr_occupation extends Da_hr_occupation {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_prefix` 
	...
	*/
	function get_all_by_active($active="Y"){
		$sql = "SELECT * 
				FROM ".$this->hr_db.".hr_base_occupation 
				WHERE pf_active = '$active'";
		$query = $this->hr->query($sql);
		return $query;
	}
	function get_gender($active="Y"){
		$sql = "SELECT * 
