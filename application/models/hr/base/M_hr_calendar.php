<?php
/*
* m_hr_manage_calendar
* Model for Manage about hr_base_manage_calendar Table.
* @Author Jiradat Pomyai
* @Create Date 30/05/2024
*/
include_once("Da_hr_calendar.php");

class m_hr_calendar extends Da_hr_calendar
{
	...
	*/
	...
	{
		$sql = "UPDATE " . $this->hr_db . ".`hr_base_manage_calendar` 
				SET `voc_active` = '" . $active . "' 
				WHERE `hr_base_manage_calendar`.`voc_id` = ?;";
	...
	*/
	function get_all_by_active($delete = "2")
	{
		$sql = "SELECT * 
				...
				WHERE c.clnd_active != '$delete'
				ORDER BY c.clnd_active DESC,c.clnd_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
	...
	{
		$sql = "SELECT * 
				FROM " . $this->hr_db . ".hr_base_calendar_type";
		$query = $this->hr->query($sql);
		return $query;
	}
	function get_filter_year()
	{
		$sql = "SELECT clnd_year
	...
	function get_all_by_year($year, $delete = "2")
	{
		$sql = "SELECT * 
       			 FROM {$this->hr_db}.hr_base_calendar AS c
       			 INNER JOIN {$this->hr_db}.hr_base_calendar_type AS ct ON c.clnd_type_date = ct.lct_id 
       			 WHERE " . ($year == 'all' ? "" : "c.clnd_year = '$year' AND") . " 
       			 c.clnd_active != '$delete' 
        		 ORDER BY c.clnd_active DESC, c.clnd_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
