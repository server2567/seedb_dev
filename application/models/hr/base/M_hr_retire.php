...
*/
include_once("Da_hr_retire.php");

class m_hr_retire extends Da_hr_retire {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_retire` 
				SET `retire_active` = '".$active."' 
				WHERE `hr_base_retire`.`retire_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE retire_active != '$delete'
		ORDER BY retire_name DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
