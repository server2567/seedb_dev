<?php
/*
* m_hr_external_service
* Model for Manage about hr_base_external_service Table.
* @Author Tanadon Tangjaimongkhon
* @Create Date 19/09/2024
*/
include_once("Da_hr_external_service.php");

class m_hr_external_service extends Da_hr_external_service {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_external_service` 
				SET `exts_active` = '".$active."' 
				WHERE `hr_base_external_service`.`exts_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE exts_active != '$delete'
		ORDER BY exts_active DESC,exts_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
	...
	*/
	function json_prefix($cond='', $aColumns='', $sWhere='', $sOrder='', $sLimit='') { 
		...
		$sql = "SELECT
					SQL_CALC_FOUND_ROWS ".str_rehire(" , ", " ", implode(", ", $aColumns))."
				FROM 
