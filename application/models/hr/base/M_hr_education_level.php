<?php
/*
* m_hr_education_level
* Model for Manage about hr_base_education_level Table.
* @Author Jiradat Pomyai
* @Create Date 30/05/2024
*/
include_once("Da_hr_education_level.php");

class m_hr_education_level extends Da_hr_education_level {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_education_level` 
				SET `edulv_active` = '".$active."' 
				WHERE `hr_base_education_level`.`edulv_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		$sql = "SELECT * 
				...
				WHERE edulv_active != '$delete'
				ORDER BY edulv_active DESC,edulv_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
