...
*/
include_once("Da_hr_admin_position.php");

class m_hr_admin_position extends Da_hr_admin_position {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_admin_position` 
				...
				WHERE `hr_base_admin_position`.`admin_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE admin_active != '$delete'
		ORDER BY admin_active DESC,admin_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
