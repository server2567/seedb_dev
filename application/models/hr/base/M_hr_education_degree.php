<?php
/*
* m_hr_education_degree
* Model for Manage about hr_base_education_degree Table.
* @Author Jiradat Pomyai
* @Create Date 30/05/2024
*/
include_once("Da_hr_education_degree.php");

class m_hr_education_degree extends Da_hr_education_degree {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_education_degree` 
				SET `edudg_active` = '".$active."' 
				WHERE `hr_base_education_degree`.`edudg_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		$sql = "SELECT * 
				...
				WHERE edudg_active != '$delete'
				ORDER BY edudg_active DESC,edudg_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
