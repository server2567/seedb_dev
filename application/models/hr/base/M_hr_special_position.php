<?php
/*
* m_hr_special_position
* Model for Manage about hr_base_special_position Table.
* @Author Jiradat Pomyai
* @Create Date 30/05/2024
*/
include_once("Da_hr_special_position.php");

class m_hr_special_position extends Da_hr_special_position {
	...
	*/
	function update_active($active){
		$sql = "UPDATE ".$this->hr_db.".`hr_base_special_position` 
				SET `spcl_active` = '".$active."' 
				WHERE `hr_base_special_position`.`spcl_id` = ?;";
	...
	*/
	function get_all_by_active($delete="2"){
		...
		WHERE spcl_active != '$delete'
		ORDER BY spcl_active DESC,spcl_id DESC";
		$query = $this->hr->query($sql);
		return $query;
	}
