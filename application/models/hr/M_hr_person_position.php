...
class M_hr_person_position extends Da_hr_person_position
{
	...
	*/
	...
	// get_person_position_history_by_ps_id
	function get_person_position_timeline($ps_id)
	{
		// Query ข้อมูลทั้งหมดจากฐานข้อมูล
		$sql = "SELECT 
					p.*,
					hire.hire_name as hire_name,
					retire.retire_name as retire_name,
					alp.alp_name as alp_name
				FROM 
					see_hrdb.hr_person_position_history p
				LEFT JOIN see_hrdb.hr_base_adline_position as alp 
					ON alp.alp_id = p.hipos_pos_alp_id
				LEFT JOIN see_hrdb.hr_base_retire as retire 
					ON retire.retire_id = p.hipos_pos_retire_id
				LEFT JOIN see_hrdb.hr_base_hire as hire 
					ON hire.hire_id = p.hipos_pos_hire_id
				WHERE 
					p.hipos_ps_id = ?
				ORDER BY 
					p.hipos_pos_dp_id, p.hipos_id ASC";  // เรียงตาม ASC

		$query = $this->hr->query($sql, array($ps_id));
		$results = $query->result_array();

		// จัดกลุ่มข้อมูลตาม hipos_pos_dp_id
		$groupedData = [];
		foreach ($results as $row) {
			$dp_id = $row['hipos_pos_dp_id'];

			if (!isset($groupedData[$dp_id])) {
				$groupedData[$dp_id] = [];
			}

			$groupedData[$dp_id][] = $row;
		}

		// ฟังก์ชันสำหรับจัดการสถานะที่ติดกัน และกำหนด hipos_start_date จากแถวแรก
		function filter_consecutive_statuses($rows)
		{
			$filtered = [];
			$previous_status = null;
			$current_start_date = null; // เก็บค่า hipos_start_date ของแถวแรกในสถานะเดียวกัน

			foreach ($rows as $row) {
				$current_status = $row['hipos_pos_status'];

				// หากสถานะเปลี่ยนไป
				if ($previous_status !== $current_status) {
					$current_start_date = $row['hipos_start_date']; // บันทึก start_date ของแถวแรกในสถานะใหม่
					$row['hipos_start_date'] = $current_start_date; // ใช้ start_date ของตัวเอง
					$filtered[] = $row; // เพิ่มแถวในผลลัพธ์
				} else {
					// หากสถานะเดียวกัน ใช้ start_date จากแถวแรกในกลุ่มเดิม
					$row['hipos_start_date'] = $current_start_date;
					$filtered[count($filtered) - 1] = $row; // อัปเดตแถวสุดท้ายในผลลัพธ์
				}
		...
		$finalResult = [];
		foreach ($groupedData as $dp_id => $rows) {
			// กรองแถวที่มีสถานะซ้ำกัน
			$filteredRows = filter_consecutive_statuses($rows);

			// แปลงเป็น JSON
			$formattedRows = array_map(function ($row) {
				return [
					'id' => $row['hipos_id'] ?? '-',
					'hipos_pos_id' => $row['hipos_pos_id'] ?? '-',
					'hipos_ps_id' => $row['hipos_ps_id'] ?? '-',
					'hipos_pos_dp_id' => $row['hipos_pos_dp_id'] ?? '-',
					'hipos_pos_ps_code' => $row['hipos_pos_ps_code'] ?? '-',
					'hire_name' => $row['hire_name'] ?? '-',
					'hipos_pos_admin_id' => $row['hipos_pos_admin_id'] ?? '-',
					'alp_name' => $row['alp_name'] ?? '-',
					'hipos_pos_spcl_id' => $row['hipos_pos_spcl_id'] ?? '-',
					'retire_name' => $row['retire_name'] ?? '-',
					'hipos_pos_status' => $row['hipos_pos_status'] ?? '-',
					'hipos_pos_out_desc' => $row['hipos_pos_out_desc'] ?? '-',
					'hipos_pos_attach_file' => $row['hipos_pos_attach_file'] ?? '-',
					'hipos_pos_work_start_date' => $row['hipos_pos_work_start_date'] ?? '-',
					'hipos_pos_work_end_date' => $row['hipos_pos_work_end_date'] ?? '-',
					'hipos_start_date' => $row['hipos_start_date'] ?? '-',
					'hipos_end_date' => $row['hipos_end_date'] ?? '-',
					'hipos_pos_count_work' => $row['hipos_pos_count_work'] ?? '-',
					'hipos_update_user' => $row['hipos_update_user'] ?? '-',
					'hipos_update_date' => $row['hipos_update_date'] ?? '-'
				];
			}, $filteredRows);

			// Reverse ลำดับก่อนใส่ในผลลัพธ์
			$formattedRows = array_reverse($formattedRows);

			$finalResult[] = [
				'hipos_pos_dp_id' => $dp_id,
				'grouped_data' => $formattedRows
			];
		}
		// ส่งผลลัพธ์กลับเป็น JSON
		return json_encode($finalResult, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
	}
