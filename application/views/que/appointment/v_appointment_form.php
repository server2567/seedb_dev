          ...
          <div class="card mb-0">
            ...
            <div class="card-body">
              ...
              <div class="row justify-content-md-center mt-1 mb-3">
                  ...
                  */
                  ...
                  <div class="col-4 col-md-3 col-lg-3">
                    ...
                    <?php if (!isset($get_appointment['apm_app_walk']) || $get_appointment['apm_app_walk'] == 'A' || $get_appointment['apm_app_walk'] == '') { ?>
                      ...
                      <span class='text-danger'>*</span></label>
                    <div class="mb-4 input-group-md">
                    <input type="time" id="apm_time" class="form-control timepicker_th" >
                ...
                <hr class="">
                <div class="row mt-3 px-4" id="appointmentDetails"  style="<?php if (isset($get_appointment['apm_app_walk']) && $get_appointment['apm_app_walk']=='A'){echo "display: flex;";}else{echo "display: none;";}  ?> ">
                 <h5 class="mb-4 font-weight-600 font-20">
                  <?php if (empty($get_appointment)) { ?>
                ...
                <div class="row mt-3">
                  <div class="col-12 col-md-8 col-lg-6">
                    <label for="apm_stde_id" class="fw-bold">
                      ...
                      <span class='text-danger'>*</span></label>
                    <div class="mb-4 input-group-md">
                      <select id="apm_stde_id" class="form-select select2-multiple" data-placeholder="-- กรุณาเลือกแผนก --"  name="apm_stde_id" multiple required>
                        <option value=""></option>
                        <?php foreach ($get_structure_detail as $key => $sd) { ?>
                          <option value="<?php echo $sd['stde_id']; ?>"
                                  data-department='<?php echo $sd['stde_id']; ?>' 
                ...
                <div class="row px-4">
                  <div class="col-12 col-md-9 col-lg-9">
                    ...
                    <?php } ?>
                    <div class="mb-4 input-group-md input-doctor">
                      <select id="apm_ps_id-rm" class="form-select select2" data-placeholder="-- กรุณาเลือกแพทย์ --" name="apm_ps_id" required>
                    ...
                    <div id="doctor-select-container" class="mb-4 input-group-md">
    <!-- Dynamic doctor selects will be inserted here -->
</div>
                  </div>
                  </div>
              ...
              <div class="row mt-1 mb-4">
                <div class="col-12 col-md-12 col-lg-11">
                  <?php if (!isset($get_appointment['apm_app_walk']) || $get_appointment['apm_app_walk'] == '') { ?>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const apmDateInput = document.querySelector('#apm_date');

    // Function to compare dates and toggle display
    function checkDateAndToggleDisplay() {
        // Get today's date in the desired format (e.g., 'YYYY-MM-DD')
        const today = new Date().toISOString().split('T')[0];

        // Get the appointment date from the input field (in DD/MM/YYYY format)
        const apmDate = apmDateInput.value;

        // Split the appointment date into day, month, buddhistYear
        const [day, month, buddhistYear] = apmDate.split('/');

        // Convert Buddhist year to Gregorian year by subtracting 543
        const year = buddhistYear - 543;

        // Reformat the date to YYYY-MM-DD for comparison
        const apmDateFormatted = `${year}-${month}-${day}`;

        const apmAppWalk = '<?php echo isset($get_appointment['apm_app_walk']) ? $get_appointment['apm_app_walk'] : ''; ?>';
        // Compare the dates and toggle display
        if (apmAppWalk === 'A'|| apmAppWalk === 'W' || apmDateFormatted !== today) {
            document.getElementById('appointmentDetails').style.display = 'flex';
        } else {
    ...
    checkDateAndToggleDisplay();

    // Attach the checkDateAndToggleDisplay function to the onchange event
    apmDateInput.addEventListener('change', checkDateAndToggleDisplay);
});
  document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php if(empty($get_appointment)) {
      echo "const patientInfoIcon = document.getElementById('patientInfoIcon');";
    }?>
    ...
    function updateDynamicContent() {
      if (gridRadios1.checked) {
        ...
        <?php if(empty($get_appointment)) { echo "
  ...
  document.addEventListener("DOMContentLoaded", function (event) {
    // Get the current time and format it as "H:i"
    ...
    // Initialize flatpickr with a slight delay to ensure proper initialization
    setTimeout(function() {
        flatpickr(".timepicker_th", {
            ...
            dateFormat: "H:i",
            time_24hr: true,
            minTime: formattedTime, // Set minimum time to current time
            defaultDate: formattedTime, // Set default time to current time
            allowInput: true,
        });
  ...
  $(document).ready(function() {
    ...
    // Get the selected date from datepicker and current date
    var dateString = $('.datepicker_th').val();

    var selectedDateParts = dateString.split('/');
    var selectedYear = parseInt(selectedDateParts[2], 10) - 543; // Convert Thai year to Gregorian year
    var selectedMonth = selectedDateParts[1].padStart(2, '0'); // Ensure month is two digits
    var selectedDay = selectedDateParts[0].padStart(2, '0'); // Ensure day is two digits

    // Manually construct the selected date in ISO format (YYYY-MM-DD)
    var selectedDateFormatted = selectedYear + '-' + selectedMonth + '-' + selectedDay;

    var currentDate = new Date(); // Get current date
    var formattedCurrentDate = currentDate.toISOString().split('T')[0]; // Convert current date to YYYY-MM-DD format
  ...
  $(document).ready(function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (data.exists) {
            ...
            Swal.fire({
              title: 'ไม่พบข้อมูล',
              html: 'เลขบัตรประชาชนไม่มีอยู่ในระบบ <br>สามารถดำเนินการลงทะเบียนข้อมูลผู้ป่วยได้ที่หัวข้อ 3',
              icon: 'error',
              customClass: {
                htmlContainer: 'swal2-html-line-height'
              },
    ...
    function fetchDoctors(departmentId, showAll = false) {
      ...
      $('.input-doctor').addClass('d-none');
      // Clear previous options
      // doctorSelect.html('<option value="0">ไม่ต้องการระบุแพทย์</option>');
      const container = $('#doctor-select-container');
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          // } else {
            ...
            let departmentCounter = 5.1;
            const appointmentPsId = "<?php if (isset($get_appointment['ps_id'])) {echo $get_appointment['ps_id'];} ?>";
                $.each(data, function(index, doctor) {
                    const departmentIds = doctor.stde_id;
                    if (!doctorsByDepartment[departmentIds]) {
                        doctorsByDepartment[departmentIds] = [];
                    }
                    doctorsByDepartment[departmentIds].push(doctor);
                });

                // Create and populate selects for each department
                $.each(departmentId, function(index, departmentIds) {
                  const departmentName = $(`#apm_stde_id option[value='${departmentIds}']`).text();
                    
                    const labelElement = $('<label>', {
                        class: 'fw_bold mb-1',
                        html: `${departmentCounter.toFixed(1)} ${departmentName} <span class="text-danger">*</span>`
                    });

                    const selectElement = $('<select>', {
                        class: 'form-select select-test ',
                        'data-placeholder': '-- กรุณาเลือกแพทย์ --',
                        name: `apm_ps_id[${index}]`,  // Array notation for storing multiple selections
                        id: `apm_ps_id_${index}` ,
                        'data-dep': departmentName
                    });

                    selectElement.append('<option value="0" name="0">ไม่ต้องการระบุแพทย์</option>');
                    
                    $.each(doctorsByDepartment[departmentIds] || [], function(index, doctor) {
                        selectElement.append($('<option>', {
                            value: doctor.ps_id,
                            text: `${doctor.pf_name_abbr} ${doctor.ps_fname} ${doctor.ps_lname}`,
                            selected: doctor.ps_id == appointmentPsId 
                        }));
                    });

                    container.append(labelElement).append(selectElement);
                    departmentCounter += 0.1;
                });
    ...
    //   });
    // }
    <?php if (!empty($get_appointment['apm_stde_id'])) { ?>
    const selectedDepartment = $('#apm_stde_id').val();
  ...
  function validateFormData(formData) {
    ...
    const requiredFields = ['idCard', 'prefix', 'firstName', 'lastName', 'phoneNumber', 'gridRadios', 'apm_dp_id', 'apm_date', 'apm_time'];
...
function saveFormData() {
  tinymce.triggerSave();
  let doctorSelections = [];
  
  let doctorHtml = '';

  // Loop through each select element with a unique ID
  $('select[id^="apm_ps_id_"]').each(function() {
    const selectedValue = $(this).val();
    const selectedDoctorName = $(this).find('option:selected').text();
    const associatedDepartmentName = $(this).attr('data-dep');
    console.log("Selected Value:", selectedValue);
    console.log("Selected Doctor Name:", selectedDoctorName);
    console.log("Associated Department Name:", associatedDepartmentName);
    // Store selected doctor values
    if (selectedValue) {
      doctorSelections.push(selectedValue);

      // Create HTML for selected doctors
      doctorHtml += `
        <div class="col-5 text-right">
          <strong>แพทย์ที่นัดหมาย (${associatedDepartmentName}):</strong>
        </div>
...
let departmentHtml = '';
// Create HTML for department information if a department is selected
if (departmentName) {
  departmentHtml = `
    <div class="col-5 text-right">
      <strong>แผนก:</strong>
  ...
  Swal.fire({
    ...
    html: `
      <div class="row">
        ...
        ${departmentHtml}
        ${doctorHtml} <!-- Doctor HTML inserted here if not empty -->
        <div class="col-5 text-right">
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url: '<?php echo site_url("que/Appointment/insert_appointment") ?>',
        type: 'POST',
...
console.log(formData); 
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      // หากผู้ใช้กดยืนยัน ให้บันทึกข้อมูลลงฐานข้อมูล
      $.ajax({
        url: '<?php echo site_url("que/Appointment/insert_appointment_api") ?>',
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify(formData),
        success: function(response) {
