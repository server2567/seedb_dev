...
<div class="col-md-12">
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="card">
                <div class="accordion">
                    <!-- Accordion ค้นหารายการลงเวลาทำงาน -->
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAdd" aria-expanded="true" aria-controls="collapseAdd">
                                <i class="bi-search icon-menu"></i><span> ค้นหารายการลงเวลาทำงาน</span>
                            </button>
                        </h2>
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button accordion-button-table" type="button">
                                <i class="bi-people icon-menu"></i>จัดการลงเวลาทำงาน
                            </button>
                        </h2>
...
var filter_end_date = '';
var filter_start_date_public = '';
var filter_end_date_public = '';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(default_show == 1){
        $('#fillter_select_dp_id').on('change', function() {
...
function initializeCalendar() {
    ...
    event_calendar = new EventCalendar(document.getElementById('event_calendar'), {
        ...
        datesSet: function(info) {
            ...
            // ตรวจสอบว่าค่า timework_date_open อยู่ในช่วง filter_start_date และ filter_end_date
            // if (new Date('<?php echo $timework_date_open; ?>') >= new Date(filter_start_date) && new Date('<?php echo $timework_date_open; ?>') <=  new Date(filter_end_date)) {
            //     $("#button_confirm_timework_calendar_normal").fadeIn(300);
            // } else {
            //     $("#button_confirm_timework_calendar_normal").fadeOut(300);
            // }
        }
...
function export_print_person(ps_id, isPublic) {
    // กำหนดวันที่ตามค่า isPublic
    var startDate = isPublic == 0 ? filter_start_date : filter_start_date_public;
    var endDate = isPublic == 0 ? filter_end_date : filter_end_date_public;

    // เปิดหน้าต่างใหม่เพื่อส่งออกข้อมูล
    window.open('<?php echo site_url($controller_dir . 'export_print_timework_calendar'); ?>/' + ps_id + '/' + isPublic + '/approver' + '/' + twpp_dp_id + "/" + startDate + "/" + endDate, '_blank');
}

function export_excel_person(ps_id, isPublic) {
    // กำหนดวันที่ตามค่า isPublic
    var startDate = isPublic == 0 ? filter_start_date : filter_start_date_public;
    var endDate = isPublic == 0 ? filter_end_date : filter_end_date_public;

    // เปิดหน้าต่างใหม่เพื่อส่งออกข้อมูล
    window.open('<?php echo site_url($controller_dir . 'export_excel_timework_calendar'); ?>/' + ps_id + '/' + isPublic + '/approver' + '/' + twpp_dp_id + "/" + startDate + "/" + endDate, '_blank');
}

function export_pdf_person(ps_id, isPublic) {
    // กำหนดวันที่ตามค่า isPublic
    var startDate = isPublic == 0 ? filter_start_date : filter_start_date_public;
    var endDate = isPublic == 0 ? filter_end_date : filter_end_date_public;

    // เปิดหน้าต่างใหม่เพื่อส่งออกข้อมูล
    window.open('<?php echo site_url($controller_dir . 'export_pdf_timework_calendar'); ?>/' + ps_id + '/' + isPublic + '/approver' + '/' + twpp_dp_id + "/" + startDate + "/" + endDate, '_blank');
}
...
function handleDateClick(info) {
    ...
    if (actor_type == "head_structure" && selectedStartDate < firstDayCurrentMonth) {
        dialog_error({
            'header': text_toast_default_error_header,
    ...
    if (actor_type == "head_structure" && selectedStartDate > lastDayCurrentMonth) {
        dialog_error({
            'header': text_toast_default_error_header,
...
function handleSelect(info) {
    ...
    if (actor_type == "head_structure" && selectedEndDate < firstDayCurrentMonth) {
...
function handleEventResize(info) {
    ...
    // ตรวจสอบว่ามีข้อมูลที่จำเป็นครบถ้วนหรือไม่
    if (actor_type == "head_structure" && new Date(selectedStartDate) < firstDayCurrentMonth) {
        dialog_error({
            'header': text_toast_default_error_header,
            'body': 'ไม่สามารถจัดการข้อมูลย้อนหลังได้'
        });
...
function confirm_timework_calendar(){
    ...
    if (actor_type == "head_structure" && event_calendar.getEvents().length == 0) {
        dialog_error({
            'header': text_toast_default_error_header,
...
<div class="modal fade" id="modal_confirm_timework_calendar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">ยืนยันการลงเวลาการทำงาน</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
        <ul class="list-group">
