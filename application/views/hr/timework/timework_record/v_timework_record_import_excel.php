...
<style>
    #loading {
        ...
        align-items: center;
        z-index: 1050;
        /* ค่าที่สูงกว่าส่วนอื่นเพื่อให้แสดงอยู่ข้างหน้า */
    }
...
<script>
    ...
    function populateTable(table, data) {
        ...
        for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
            ...
            // ตรวจสอบว่าข้อมูล index ตั้งแต่ 4 เป็นต้นไปเป็นค่าว่างทั้งหมดหรือไม่
            const isRowEmpty = row.slice(5).every(cell => cell == '');
            if (isRowEmpty) {
                continue;
            }
            ...
            row.forEach((cell, colIndex) => {
                ...
                td.textContent = cell || '';
                td.id = `row-${tableCounter}-${status_index}-col-${colIndex}`;
                tr.appendChild(td);
            });

            const statusTd = document.createElement('td');
            statusTd.id = `row-${tableCounter}-${status_index}-status`;
            ...
            .then(function(isValid) {
                ...
                if (isValid) {
                    dialog_success({
                        ...
                        'body': "การตรวจสอบเสร็จสิ้น: ข้อมูลถูกต้อง"
                    });
