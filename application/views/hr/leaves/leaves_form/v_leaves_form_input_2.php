...
/*
	...
	* หน้าจอแบบฟอร์มการลาตามประเพณี
	* @input lhis_leave_id = 2
	* $output -
                ...
                <div class="accordion-body">
                    ...
                    <form id="leaves_form_input" class="needs-validation" method="post" action="<?php echo site_url() . "/" . $controller; ?>leaves_save">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-12" id="output_leaves_table">
                        ...
                        <!-- ประวัติการลาตามประเพณีครั้งล่าสุด -->
                        <?php
                        $address_text = "";
                        ?>
...
<script>
    ...
    function leaves_save_form() {
        ...
        var selectedValues = [];
        $(".leaves_clnd_id").each(function(index) {
            // ตรวจสอบว่าตัวเลือกนี้มองเห็นได้หรือไม่
            if ($(this).closest('td').is(':visible')) {
                var value = $(this).val();
                var warningDiv = $("#show_warning_clnd_" + index); // เลือก div แสดงข้อความเตือน
                if (value === '' || value === null) {
                    isValid = false;
                    var warningText = "กรุณาเลือกวันหยุดประเพณี<br>";
                    text_valid = warningText;
                    warningDiv.html(
                        "<span class='badge bg-danger text-white'><i class='bi bi-exclamation-octagon me-1 font-16'></i> " +
                        warningText +
                        "</span>"
                    ).show(); // แสดงข้อความเตือน
                    $(this).addClass('is-invalid'); // เพิ่ม class แสดงความผิดพลาด
                } else if (selectedValues.includes(value)) {
                    isValid = false;
                    var duplicateText = "ไม่สามารถเลือกวันหยุดประเพณีซ้ำได้ โปรดเลือกใหม่<br>";
                    text_valid = duplicateText;
                    warningDiv.html(
                        "<span class='badge bg-danger text-white'><i class='bi bi-exclamation-octagon me-1 font-16'></i> " +
                        duplicateText +
                        "</span>"
                    ...
                    selectedValues.push(value); // เพิ่มค่าเข้าไปในอาร์เรย์เพื่อตรวจสอบซ้ำ
                    $(this).removeClass('is-invalid').addClass('is-valid'); // ลบ class แสดงความผิดพลาด
                    warningDiv.hide(); // ซ่อนข้อความเตือน
                }
            }
        });
        ...
        // ตรวจสอบผลรวมเวลาว่าต้องมากกว่า 1 ชั่วโมง
        if (day == 0 && hour == 0 && minute == 0) {
            isValid = false;
            var timeText = "ผลรวมจำนวนต้องมากกว่า 1 ชั่วโมง<br>";
            text_valid += timeText;
        } else if (day == 0 && hour < 1) {
            isValid = false;
            var hourText = "ผลรวมจำนวนต้องมากกว่า 1 ชั่วโมง<br>";
            text_valid += hourText;
        }
    ...
    function get_base_calendar_for_select_leaves(startDate, endDate) {
        $.ajax({
            url: '<?php echo site_url() . "/" . $controller; ?>get_base_calendar_for_leave',
            type: 'POST',
            data: {
            ...
            success: function(response) {
                ...
                // ค้นหา select ทั้งหมดที่มี class 'leaves_clnd_id'
                const selectElements = document.querySelectorAll(".leaves_clnd_id");

                selectElements.forEach((selectElement) => {
                    // ใช้ Set แยกสำหรับแต่ละ select
                    const addedOptions = new Set();

                    // ล้างข้อมูลเดิมใน select
                    selectElement.innerHTML = "<option value='' selected>-- กรุณาเลือกวันหยุดประเพณี --</option>";

                    // เติม option ใหม่จาก data
                    data.forEach((holiday) => {
                        if (!addedOptions.has(holiday.clnd_id)) { // ตรวจสอบว่าค่า clnd_id ยังไม่ถูกเพิ่ม
                            const option = document.createElement("option");
                            option.value = holiday.clnd_id;
                            option.textContent = holiday.clnd_name;
                            selectElement.appendChild(option);
                            addedOptions.add(holiday.clnd_id); // เพิ่มค่า clnd_id เข้าไปใน Set
                        }
                    });
                });
    ...
    function generate_leaves_table() {
        ...
        var table = "<table class='table table-bordered' width='100%'>" +
            ...
            "<th class='text-center' scope='col' rowspan='2' width='15%'>วันหยุดตามประเพณี</th>" +
        ...
        for (var i = 0; i <= diffDays; i++) {
            ...
            table += "<tr>" +
                ...
                "</td>" +
                "<td id='td_base_calendar_" + i + "'>" +
                    "<select class='form-select leaves_clnd_id' data-placeholder='-- กรุณาเลือกวันหยุดประเพณี --' name='leaves_clnd_id[]' id='leaves_clnd_id_" + i + "' ></select>" +
                    "<div class='text-center mt-2' id='show_warning_clnd_" + i + "' style='display: none;'><span class='badge bg-danger text-white'><i class='bi bi-exclamation-octagon me-1 font-16'></i></span></div>" +
                "</td>" +
                "<td>" +
        ...
        get_base_calendar_for_select_leaves(startDateValue, endDateValue);
