                    ...
                    <form class="row g-3" method="post" action="<?php echo base_url(); ?>index.php/ums/SyncHRsingle">
                        <div class="col-md-7">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-3">
                                    <label id="budget_year_display" for="" class="form-label"><?php echo $budget_year + 543; ?></label>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    document.getElementById("work_age_display").innerHTML = calWorkAge(`<?= $target_user_leave_summary['final_found'] ?>`, <?= $target_user_leave_summary['work_experience_days'] ?>);
    ...
    function getData() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let targetIndex = data.base_info.pos_work_start_date_with_dp_id.findIndex((item) => {
                    return item.pos_dp_id == $('#select_dp_id').val();
                });

                $("#ps_name_display").html(`${data.target_user_leave_summary[0].pf_name}${data.target_user_leave_summary[0].ps_fname} ${data.target_user_leave_summary[0].ps_lname}`);
                $("#hire_abbr_display").html(`${data.base_info.pos_work_start_date_with_dp_id[targetIndex].hire_abbr}`);
                $("#detail_display").html(`${data.base_info.pos_work_start_date_with_dp_id[targetIndex].detail}`);
                $("#work_age_display").html(`-`);
