...
<style>
    ...
    #examination-result-current-table th, 
    #examination-result-current-table td {
        border-bottom-width: var(--bs-border-width);
    }
    #examination-result-current-table tr:last-child td {
                ...
                <div class="card" <?php echo $is_officer ? 'style="margin-bottom: 0px;"' : ''; ?>>
                    ...
                    <div class="row m-3">
                        <div class="col-md-3 p-5 profile-card d-flex align-items-center justify-content-center">
                            <?php if (isset($detail->ptd_img_type) && isset($detail->ptd_img_code) && !empty($detail->ptd_img_type) && !empty($detail->ptd_img_code)) { ?>
                                <img style="width:250px; height:250px; object-fit: cover;" class="profileImage avatar-img rounded-circle border border-white border-3 shadow" src="data:image/<?php echo $detail->ptd_img_type; ?>;base64,<?php echo $detail->ptd_img_code; ?>" alt="">
                            <?php } else { ?>
                                ...
                                <!-- <img class="img-fluid border rounded-circle" style="border-width: 2px; border-color: black;" id="profile_picture" src="<?php echo site_url() ?>/hr/getIcon?type=profile/profile_picture&image=profile_picture_4.jpg">  -->
                            <?php } ?>
                        </div>
                    ...
                    <!-- DIM current examination result -->
                    <div class="row m-3">
                        <div class="col-md-12">
                            <h5 class="weight-600 font-20"> ผลตรวจจากเครื่องมือหัตถการ </h5>
                        </div>
                        <div class="col-md-12 container-tool">
                            <table id="examination-result-current-table" class="table table-bordered" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%" class="text-center">#</th>
                                        <th width="10%" class="text-center">Visit</th>
                                        <th width="10%" class="text-center">วันที่ตรวจ</th>
                                        <th width="20%" class="text-center">ห้องปฏิบัติการ</th>
                                        <th width="20%" class="text-center">เครื่องมือหัตถการ</th>
                    ...
                    <!-- DIM history examination result -->
                    <div class="row m-3">
                        <div class="col-md-12">
                            <h5 class="weight-600 font-20"> ประวัติการตรวจเครื่องมือหัตถการ </h5>
                        </div>
                        <div class="col-md-12 container-tool">
                            <table id="examination-result-history-table" class="table table-bordered" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%" class="text-center">#</th>
                                        <th width="10%" class="text-center">Visit</th>
                                ...
                                <tbody>
                                    <?php 
                                        // Initialize an array to track rowspan counts for the combination of apm_visit and apm_date
                                        $visit_date_rowspan = [];

                                        // First, calculate the rowspan for each distinct pair of apm_visit and apm_date
                                        foreach ($history_apms as $row) {
                                            if ($row['apm_visit'] == $detail->apm_visit) {
                                                // Skip rows with apm_visit equal to $detail->apm_visit
                                                continue;
                                            }
                                            $visit_date_key = $row['apm_visit'] . '_' . $row['apm_date'];

                                            // Count rowspan for the combination of apm_visit and apm_date
                                            if (isset($visit_date_rowspan[$visit_date_key])) {
                                                $visit_date_rowspan[$visit_date_key]++;
                                            } else {
                                                $visit_date_rowspan[$visit_date_key] = 1;
                                            }
                                        }

                                        // Track which combination of apm_visit and apm_date has been processed
                                        $visit_date_processed = [];

                                        // Now loop through the data and apply rowspan
                                        $order_no = 1;
                                        foreach ($history_apms as $i => $row) {
                                            // Skip rows where apm_visit is $detail->apm_visit
                                            if ($row['apm_visit'] == $detail->apm_visit) {
                                                continue;
                                            }
                                    ...
                                    ?>
                                        <tr>
                                            <!-- Combine apm_visit and apm_date columns and apply rowspan -->
                                            <?php if (!isset($visit_date_processed[$visit_date_key])) { ?>
                                                <td class="text-center align-content-center" rowspan="<?php echo $visit_date_rowspan[$visit_date_key]; ?>"><?php echo $order_no; ?></td>
                                                <td class="text-center align-content-center" rowspan="<?php echo $visit_date_rowspan[$visit_date_key]; ?>">
                                                    <?php echo $row['apm_visit']; ?>
                                                </td>
                                                <td class="text-center align-content-center" rowspan="<?php echo $visit_date_rowspan[$visit_date_key]; ?>">
                                                    <?php echo convertToThaiYear($row['apm_date'], false); ?>
                                                </td>
                                            <?php $order_no++; } ?>
                                            
                                            <td class="align-content-center"><?php echo $row['rm_name']; ?></td>
                                            <td class="align-content-center"><?php echo $row['eqs_name']; ?></td>
                                            <td class="text-center align-content-center <?php echo $row['exr_status_class']; ?>">
                                                <?php echo $row['exr_status_text']; ?>
                                            </td>
                                            <td class="text-center option">
                                                <?php 
                                                    if($row['exr_status'] == 'Y') {
                                                        $params = "'".encrypt_id($row['exr_eqs_id'])."','".encrypt_id($row['apm_pt_id'])."','".encrypt_id($row['apm_stde_id'])."','".encrypt_id($row['apm_ds_id'])."'"; ?>
                                                        <button type="button" title="ดูผลตรวจ" class="btn btn-info" onclick="loadModalExrHistories(<?php echo $params; ?>)"><i class="bi-search"></i></button>
                                                <?php } ?>
                                            </td>
                                        </tr>

                                    <?php 
                                            // Mark the combination of apm_visit and apm_date as processed
                                            $visit_date_processed[$visit_date_key] = true;
                                        } 
                                    ?>
                                    <?php    
                                        // if no row
...
<script>
    ...
    $(document).ready(function() {
        ...
        //     //     $num += $exr_amount;
        //     // }
        //     let count = $('#examination-result-current-table tbody tr').length;
        //     console.log(count);
        //     default_tool_table();
        // }
        ...
        $('#tool-tbody').on('click', '.removeElement', function() {
            ...
            order_tools = $('#examination-result-current-table tbody tr').not(':has(td.dt-empty)').length;
        ...
        createDataTableServerExr('#examination-result-current-table', url)
        resetIntervalExr();
    ...
    function createDataTableServerExr(selector, url) {
        ...
        $(selector).DataTable({
            ...
            "drawCallback": function(settings) {
                ...
                // const check = $('#examination-result-current-table tbody tr:has(td.dt-empty)').length;
                // if(check == 0 && !is_first_load_exr) {
                //     order_tools = $('#examination-result-current-table tbody tr').not('.dataTables_empty').length;
    ...
    function reloadDataTableExr() {
        $('#examination-result-current-table').DataTable().ajax.reload(null, false); // false to stay on the current page
    }
    ...
    function loadModalExrHistories(exr_eqs_id, apm_pt_id, apm_stde_id, ds_id = null) {
        $('#modal-load').modal('show');
        $('#modal-content-loading').html(`
            <div class="center-container">
                <div class="spinner-border text-info" role="status">
        ...
        $.ajax({
            url: "<?php echo site_url('/ams/Notification_result/Notification_result_get_all_docs_by_tool'); ?>/" + exr_eqs_id + '/' + apm_pt_id + '/' + apm_stde_id + '/' + ds_id,
            method: "GET",
            success: function(data) {
                $('#examination-result-doc').html(data);
            },
