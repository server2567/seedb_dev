...
<div class="modal fade" id="modalPatientServiceTime" style="width:100%; height:100%;" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered" >
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">[WTS-G3] จำนวนผู้ป่วยที่ได้รับบริการจากแพทย์ ในระยะเวลาที่เกินเวลา และไม่เกินเวลา</h5> 
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row mt-1">
                    <div class="col-md-10">
                        <ul class="nav nav-pills" id="TimeTypeTab" role="tablist">
                    ...
                    <div class="col-md-2" style=" border-left: 1px solid #ccc !important;">
                        <label class="w-100">
                            ประเภทการบริการ
                            <select name="location-selector" id="location-selector" class="form-control">
                                <option value="">ทั้งหมด</option>
                            </select>
                        </label>
                    </div>
                </div>
                <div id="letterFilter-patient-service-time" class="row mb-3 letterFilter">
                    <div class="col-12 d-flex ">
                        <div class="btn-group" role="group">
                            ...
                            foreach ($letters as $letter) {
                                echo '<input type="checkbox" class="btn-check" id="letter-patient-service-time-' . $letter . '" value="' . $letter . '">';
                                echo '<label class="btn btn-outline-primary" for="letter-patient-service-time-' . $letter . '">' . $letter . '</label>';
                            }
                            ?>
                        </div>
                    </div>
                ...
                <script>
                    $(document).ready(function() {
                        $('#letterFilter-patient-service-time .btn-check').on('change', function() {
                ...
                <div class="row mt-1">
                    <div class="col-md-12 table-responsive">
                        <table class="table  table-bordered table-hover dataTable" width="100%" id="wtsPatientServiceTimeTable">
                            <thead>
                                <tr>
 ...
 <script>
    function getPatientServiceTimeDetail(e){
        ...
        let wtsPatientServiceTimeTable = $.fn.DataTable.isDataTable('#wtsPatientServiceTimeTable');
        if (wtsPatientServiceTimeTable) {
            $('#wtsPatientServiceTimeTable').DataTable().destroy(); // ทำลาย DataTable เดิม
        }
        ...
        document.querySelectorAll('#TimeTypeTab .nav-link').forEach(function(tab) {
            tab.addEventListener('click', function(event) {
                let clickedTab = event.target; 
                tabVal = clickedTab.getAttribute('value'); 

                $('#letterFilter-patient-service-time input[type="checkbox"]').prop('checked', false)
                $('#letterFilter-patient-service-time .btn-check').each(function() {
                    var label = $('label[for="' + $(this).attr('id') + '"]');
                    label.removeClass('btn-primary text-white').addClass('btn-outline-primary');
                });
                selectedLetters = []
                wtsPatientServiceTimeTable.ajax.reload();
            });
        });


        $('#letterFilter-patient-service-time input[type="checkbox"]').on('change', function () {
            selectedLetters = []
            $('#letterFilter-patient-service-time input[type="checkbox"]:checked').each(function () {
        ...
        const mySelect  = document.getElementById('location-selector');
        mySelect.addEventListener('change', function() {
            wtsPatientServiceTimeTable.ajax.reload();
        });


        wtsPatientServiceTimeTable = $('#wtsPatientServiceTimeTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
        ...
        var modal = document.getElementById('modalPatientServiceTime');
