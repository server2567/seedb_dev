...
<div class="modal fade" id="modalBeingServed" style="width:100%; height:100%;" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered" >
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">[WTS-C2] ผู้ป่วยที่กำลังรับบริการ</h5>
            ...
            <div class="modal-body"> 
                ...
                <div id="letterFilter-service" class="row mb-3 letterFilter">
                    <div class="col-12 d-flex ">
                        <div class="btn-group" role="group">
                            ...
                            foreach ($letters as $letter) {
                                echo '<input type="checkbox" class="btn-check" id="letter-service-' . $letter . '" value="' . $letter . '">';
                                echo '<label class="btn btn-outline-primary" for="letter-service-' . $letter . '">' . $letter . '</label>';
                            }
                            ?>
                        </div>
                    </div>
                ...
                <script>
                    $(document).ready(function() {
                        $('#letterFilter-service .btn-check').on('change', function() {
                ...
                <div class="row  mt-3">
                    <div class="col-md-12 table-responsive">
                        <table class="table  table-bordered table-hover dataTable" width="100%" id="wtsServiceTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 120px; text-align: center;">วันที่ดำเนินการ</th>
...
<script>
    function getDetailBeingServed(e) {
        toggleLoaderSearchBtn(e)
        $('#letterFilter-service input[type="checkbox"]').prop('checked', false)
        ...
        let wtsServiceTable = $.fn.DataTable.isDataTable('#wtsServiceTable');
        if (wtsServiceTable) {
            $('#wtsServiceTable').DataTable().destroy(); // ทำลาย DataTable เดิม
        }

        var activeTab = document.querySelector('#serviceTab .nav-link.active');
        var tabVal = activeTab ? activeTab.getAttribute('value') : '';
        var selectedLetters = []


        document.querySelectorAll('#serviceTab .nav-link').forEach(function(tab) {
            tab.addEventListener('click', function(event) {
                let clickedTab = event.target; 
                tabVal = clickedTab.getAttribute('value'); 

                $('#letterFilter-service input[type="checkbox"]').prop('checked', false)
                $('#letterFilter-service .btn-check').each(function() {
                    var label = $('label[for="' + $(this).attr('id') + '"]');
                    label.removeClass('btn-primary text-white').addClass('btn-outline-primary');
                });
                selectedLetters = []
                wtsServiceTable.ajax.reload();
            });
        });

        $('#letterFilter-service input[type="checkbox"]').on('change', function () {
            selectedLetters = []
            $('#letterFilter-service input[type="checkbox"]:checked').each(function () {
        ...
        wtsServiceTable = $('#wtsServiceTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "<?php echo site_url('/seedb/wts/Wts_dashboard/getBeingServedDetail'); ?>",
                type: 'POST',
                data: function (d) {
        ...
        var modal = document.getElementById('modalBeingServed');
