...
<div class="modal fade" id="modalPatientServiceTimeByDoctor" style="width:100%; height:100%;" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered" >
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">[WTS-G4] จำนวนผู้ป่วยที่ได้รับบริการทุกจุดให้บริการ ในระยะเวลาที่เกินเวลา และไม่เกินเวลา</h5> 
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row mt-1">
                    <div class="col-md-8">
                        <ul class="nav nav-pills" id="TimeTypeByDoctorTab" role="tablist">
                    ...
                    <div class="col-md-4" style=" border-left: 1px solid #ccc !important;">
                        <label class="w-100">
                            รายชื่อแพทย์
                            <select name="location-selector-ByDoctor" id="location-selector-ByDoctor" class="form-control">
                                <option value="">ทั้งหมด</option>
                            </select>
                        </label>
                    </div>
                </div>
                <div id="letterFilter-patient-service-time-ByDoctor" class="row mb-3 letterFilter">
                    <div class="col-12 d-flex ">
                        <div class="btn-group" role="group">
                            ...
                            foreach ($letters as $letter) {
                                echo '<input type="checkbox" class="btn-check" id="letter-patient-service-time-ByDoctor-' . $letter . '" value="' . $letter . '">';
                                echo '<label class="btn btn-outline-primary" for="letter-patient-service-time-ByDoctor-' . $letter . '">' . $letter . '</label>';
                            }
                            ?>
                        </div>
                    </div>
                ...
                <script>
                    $(document).ready(function() {
                        $('#letterFilter-patient-service-time-ByDoctor .btn-check').on('change', function() {
                ...
                <div class="row mt-1">
                    <div class="col-md-12 table-responsive">
                        <table class="table  table-bordered table-hover dataTable" width="100%" id="wtsPatientServiceTimeByDoctorTable">
                            <thead>
                                <tr>
 ...
 <script>
    ...
    function getPatientServiceTimeByDoctorDetail(e) {
        ...
        let wtsPatientServiceTimeByDoctorTable = $.fn.DataTable.isDataTable('#wtsPatientServiceTimeByDoctorTable');
        if (wtsPatientServiceTimeByDoctorTable) {
            $('#wtsPatientServiceTimeByDoctorTable').DataTable().destroy(); // ทำลาย DataTable เดิม
        }
        ...
        document.querySelectorAll('#TimeTypeByDoctorTab .nav-link').forEach(function(tab) {
            tab.addEventListener('click', function(event) {
                let clickedTab = event.target; 
                tabVal = clickedTab.getAttribute('value'); 

                $('#letterFilter-patient-service-time-ByDoctor input[type="checkbox"]').prop('checked', false)
                $('#letterFilter-patient-service-time-ByDoctor .btn-check').each(function() {
                    var label = $('label[for="' + $(this).attr('id') + '"]');
                    label.removeClass('btn-primary text-white').addClass('btn-outline-primary');
                });
                selectedLetters = []
                wtsPatientServiceTimeByDoctorTable.ajax.reload();
            });
        });


        $('#letterFilter-patient-service-time-ByDoctor input[type="checkbox"]').on('change', function () {
            selectedLetters = []
            $('#letterFilter-patient-service-time-ByDoctor input[type="checkbox"]:checked').each(function () {
        ...
        const mySelect  = document.getElementById('location-selector-ByDoctor');
        mySelect.addEventListener('change', function() {
            wtsPatientServiceTimeByDoctorTable.ajax.reload();
        });


        wtsPatientServiceTimeByDoctorTable = $('#wtsPatientServiceTimeByDoctorTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "<?php echo site_url('/seedb/wts/Wts_dashboard/getPatientServiceTimeByDoctorDetail'); ?>",
                type: 'POST',
                data: function (d) {
        ...
        var modal = document.getElementById('modalPatientServiceTimeByDoctor');
