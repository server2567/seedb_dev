...
<div class="modal fade" id="modalDone" style="width:100%; height:100%;" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered" >
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">[WTS-C3] ผู้ป่วยที่ดำเนินการเสร็จสิ้น</h5> 
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body"> 
                <div class="row  mb-3">
                    <div class="col-md-12">
                        <ul class="nav nav-pills" id="doneTab" role="tablist">
                ...
                <div id="letterFilter-done" class="row mb-3 letterFilter">
                    <div class="col-12 d-flex ">
                        <div class="btn-group" role="group">
                            ...
                            foreach ($letters as $letter) {
                                echo '<input type="checkbox" class="btn-check" id="letter-done-' . $letter . '" value="' . $letter . '">';
                                echo '<label class="btn btn-outline-primary" for="letter-done-' . $letter . '">' . $letter . '</label>';
                            }
                            ?>
                        </div>
                    </div>
                ...
                <script>
                    $(document).ready(function() {
                        $('#letterFilter-done .btn-check').on('change', function() {
                ...
                <div class="row  mt-3">
                    <div class="col-md-12 table-responsive">
                        <table class="table  table-bordered table-hover dataTable" width="100%" id="wtsDoneTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 120px; text-align: center;">วันที่เสร็จสิ้น</th>
...
<script>
    function getProcessIsDone(e){
        toggleLoaderSearchBtn(e)
        $('#letterFilter-done input[type="checkbox"]').prop('checked', false)
        ...
        let wtsDoneTable = $.fn.DataTable.isDataTable('#wtsDoneTable');
        if (wtsDoneTable) {
            $('#wtsDoneTable').DataTable().destroy(); // ทำลาย DataTable เดิม
        }

        var activeTab = document.querySelector('#doneTab .nav-link.active');
        var tabVal = activeTab ? activeTab.getAttribute('value') : '';
        var selectedLetters = []

        document.querySelectorAll('#doneTab .nav-link').forEach(function(tab) {
            tab.addEventListener('click', function(event) {
                let clickedTab = event.target; 
                tabVal = clickedTab.getAttribute('value'); 

                $('#letterFilter-done input[type="checkbox"]').prop('checked', false)
                $('#letterFilter-done .btn-check').each(function() {
                    var label = $('label[for="' + $(this).attr('id') + '"]');
                    label.removeClass('btn-primary text-white').addClass('btn-outline-primary');
                });
                selectedLetters = []
                wtsDoneTable.ajax.reload();
            });
        });

        $('#letterFilter-done input[type="checkbox"]').on('change', function () {
            selectedLetters = []
            $('#letterFilter-done input[type="checkbox"]:checked').each(function () {
        ...
        wtsDoneTable = $('#wtsDoneTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "<?php echo site_url('/seedb/wts/Wts_dashboard/getProcessIsDoneDetail'); ?>",
                type: 'POST',
                data: function (d) {
