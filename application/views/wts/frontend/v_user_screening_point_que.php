...
<div class="row g-3 mt-3 queue-container">
  ...
  $colorSchemes = [
    ...
    5 => ['background-color' => '#FFFFE0', 'color' => '#574900'], 
  ...
  <?php } else { ?>
    <?php foreach ($room_que as $room_id => $room) {
        $colors = $colorSchemes[$room_id] ?? $colorSchemes[1];
        ...
        <div class="card-body p-0">
          <table class="table table-borderless mb-0" style="font-size: 45px; border-bottom-left-radius: 0.5rem; border-bottom-right-radius: 0.5rem; overflow: hidden;">
            <tbody>
              ...
              <tr>
                <td class="text-center <?php echo $bgClass; ?>" style="<?php echo $rightBorderStyle; ?>">
                  <?php echo $key + 1; ?>
                </td>
              ...
              <?php if ($item['apm_pri_id'] == '1') { ?>
              <tr>
                <td class="text-center <?php echo $bgClass; ?>" style="<?php echo $rightBorderStyle; ?>"></td>
                <td colspan="2" style="padding: 20px;" class="text-center <?php echo $bgClass; ?>">
...
function updateQueue() {
  fetch('<?php echo site_url('wts/frontend/User_room_que/getQueueData/' . urlencode($stde_name)); ?>')
    ...
    .then(data => {
      ...
      if (sortedRoomQue.length === 0) {
        ...
        sortedRoomQue.forEach(([room_id, room], index) => {
          let colors = <?php echo json_encode($colorSchemes); ?>[room_id] || <?php echo json_encode($colorSchemes[1]); ?>;
              ...
              <div class="card-body p-0">
                <table class="table table-borderless mb-0" style="font-size: 45px; border-bottom-left-radius: 0.5rem; border-bottom-right-radius: 0.5rem; overflow: hidden;">
                  <tbody>
                    ${room.map((item, key) => `
                      <tr>
                        <td class="text-center ${key === 0 ? 'bg-warning-que' : ''}" style="${rightBorderStyle}">
                          ${key + 1}
                        </td>
                        <td class="text-center ${key === 0 ? 'bg-warning-que' : ''}">
                          ${item.apm_ql_code}
                      ...
                      ${item.apm_pri_id === '1' ? `
                      <tr>
                        <td class="text-center ${key === 0 ? 'bg-warning-que' : ''}" style="${rightBorderStyle}"></td>
                        <td colspan="2" style="padding: 20px;" class="text-center ${key === 0 ? 'bg-warning-que' : ''}">
                          <span class='text-danger me-2 fw-bold' style='font-size: 26px; margin-top: -30px; position: absolute; left: 34%;'>ผู้ป่วยฉุกเฉิน</span>
                        </td>
                      </tr>
                      ` : ''}
...
// Update the queue every 10 seconds
