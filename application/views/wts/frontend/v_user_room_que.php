  ...
  <h2 class="text-center">
    <?php 
      ...
      if(isset($floor) && !empty($floor)) {
        $text = "ชั้น ".$floor.' ';
        if($floor == 1) $text .=  'ภาคจักษุวิทยา (EYE) วันที่ ';
        else $text .=  ' วันที่ ';
      } else if(isset($stde_name) && !empty($stde_name)) $text = $stde_name . ' วันที่ ';
      echo $text;
      ?>
  ...
  <?php } else { 
    ...
    <div class="col-md-3 p-0">
      <div class="card mb-0" style="box-shadow: none;">
        <div class="card-header text-center fw-bold d-flex align-items-center justify-content-center" style="font-size: 44px; background-color: <?php echo $colors['background-color']; ?>; color: <?php echo $colors['color']; ?>; border: <?php echo $colors['border']; ?>;">
            <div class="card-icon rounded-circle d-flex align-items-center justify-content-center" style="color: #9b2500; background: #fbd4c7;">
              <img id="profile_picture" class="rounded-circle" src="<?php echo site_url($this->config->item('hr_dir') . "getIcon_que?type=" . $this->config->item('hr_profile_dir') . "profile_picture&image=" . (!empty($room[0]['psd_picture']) ? $room[0]['psd_picture'] : "default.png")); ?>">
            </div>
...
setInterval(updateQueue, 9999999);
