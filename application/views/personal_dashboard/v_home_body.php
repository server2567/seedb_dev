                    ...
                    <div class="card p-2 float-start" style="margin-bottom: 20px;">
                        <ul class="nav nav-pills nav-pills-2 nav-menu" role="tablist">
                            ...
                            <?php if ($this->session->userdata('us_his_id') != 0) {
                                $us_his_id = $this->session->userdata('us_his_id'); // ค่านี้เป็นตัวอย่าง คุณสามารถใช้ค่าไดนามิกได้
                                $user_ip = $this->input->ip_address();
                                $current_date = date('Y-m-d'); // รูปแบบวันที่ ปี-เดือน-วัน (เช่น 2024-09-06)
                            ?>
                                <li class="nav-item ms-2" role="presentation">
                                    <button class="nav-link d-column" id="dashboard-website-tab" type="button" onclick="window.location.href='myapp://launch?us_his_id=<?php echo $us_his_id; ?>&user_ip=<?php echo $user_ip; ?>&current_date=<?php echo $current_date; ?>';">
